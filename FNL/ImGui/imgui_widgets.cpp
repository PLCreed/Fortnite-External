...
int accent_color[4] = { 15, 15, 17, 255 };
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_hovered->second = ImClamp(it_hovered->second + (100.f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.5f)), 0.0f, 1.0f);
    ...
    it_active->second = ImClamp(it_active->second + (100.f * ImGui::GetIO().DeltaTime * (*v ? 1.f : -1.5f)), 0.0f, 1.f);
    it_active->second *= style.Alpha;

    static float fill;
    fill = ImClamp(fill + (5.f * ImGui::GetIO().DeltaTime), 0.0f, 1.f);
    fill *= style.Alpha;

    if (*v)
    {
        window->DrawList->AddRectFilled(pos + ImVec2(1, 1), pos + ImVec2(11, 11), ImColor(138, 21, 21/*157, 176, 242*/));
        window->DrawList->AddText(pos + ImVec2(14, -4), ImColor(255, 255, 255, int(200 * it_active->second)), label);
    }
    else {
        if (hovered)
        {
            window->DrawList->AddRectFilled(pos + ImVec2(1, 1), pos + ImVec2(11, 11), ImColor(138, 21, 21, 100));
            window->DrawList->AddText(pos + ImVec2(14, -4), ImColor(200, 200, 200, int(100 * it_hovered->second)), label);


        }
        else
        {
            window->DrawList->AddRectFilled(pos + ImVec2(1, 1), pos + ImVec2(11, 11), ImColor(15 / 255.f, 15 / 255.f, 15 / 255.f, g.Style.Alpha));
            window->DrawList->AddText(pos + ImVec2(14, -4), ImColor(55, 55, 55, 255), label);
        }


    }

    window->DrawList->AddRectFilledMultiColor(pos + ImVec2(1, 1), pos + ImVec2(11, 11), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, 0 / 255.f), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, 0 / 255.f), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, ImClamp(120.f / 255.f, 0.f, g.Style.Alpha)), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, ImClamp(120.f / 255.f, 0.f, g.Style.Alpha)));

    window->DrawList->AddRect(pos + ImVec2(1, 1), pos + ImVec2(11, 11), ImColor(26 / 255.f, 26 / 255.f, 26 / 255.f, g.Style.Alpha));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float size = GetWindowWidth() - 30.f;
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), clickable.Max - ImVec2(0.f, 7.f), ImColor(255, 255, 255, 255));//Background

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(180, 59, 64, 255)); //Covering Slider

    //   window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_anim->second.circle_anim, ImColor(colors::accent), 25.f);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(picker_size, picker_size - 70), 0, 0, col_black, col_black, 2.f);
    ...
    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColor(ImVec2(picker_pos.x + i * (bar_width / 6) - (i == 5 ? 1 : 0), picker_pos.y + 128), ImVec2(picker_pos.x + (i + 1) * (bar_width / 6) + (i == 0 ? 1 : 0), picker_pos.y + 128 + bar_height), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i], 2.f, i == 0 ? ImDrawFlags_RoundCornersLeft : i == 5 ? ImDrawFlags_RoundCornersRight : ImDrawFlags_RoundCornersNone);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColor(picker_pos + ImVec2(0, 141), picker_pos + ImVec2(bar_width, 141 + bar_height), col_black, user_col32_striped_of_alpha, user_col32_striped_of_alpha, col_black, 2.f);
...
bool ImGui::Tab(const char* icon, const char* label, bool selected)
{
    ...
    window->DrawList->AddRectFilled(pos, ImVec2(pos.x + 2, pos.y + size.y), ImColor(138, 21, 21, int(255 * it_filled->second)));
